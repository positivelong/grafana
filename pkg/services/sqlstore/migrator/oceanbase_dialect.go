...
func NewOceanBaseDialect() Dialect {
	d := OceanBaseDialect{}
...
func (db *OceanBaseDialect) SupportEngine() bool {
	return true
}

func (db *OceanBaseDialect) Quote(name string) string {
	return "`" + name + "`"
}

func (db *OceanBaseDialect) AutoIncrStr() string {
	return "AUTO_INCREMENT"
}

func (db *OceanBaseDialect) BooleanStr(value bool) string {
	if value {
		return "1"
	}
	return "0"
...
func (db *OceanBaseDialect) BatchSize() int {
	return 1000
}
...
func (db *OceanBaseDialect) IndexCheckSQL(tableName, indexName string) (string, []any) {
...
func (db *OceanBaseDialect) ColumnCheckSQL(tableName, columnName string) (string, []any) {
...
func (db *OceanBaseDialect) CleanDB(engine *xorm.Engine) error {
...
func (db *OceanBaseDialect) TruncateDBTables(engine *xorm.Engine) error {
	tables, err := engine.DBMetas()
	if err != nil {
		return err
	}
...
func (db *OceanBaseDialect) isThisError(err error, errcode uint16) bool {
...
func (db *OceanBaseDialect) IsUniqueConstraintViolation(err error) bool {
	return db.isThisError(err, mysqlerr.ER_DUP_ENTRY)
}

func (db *OceanBaseDialect) ErrorMessage(err error) string {
	var driverErr *mysql.MySQLError
	if errors.As(err, &driverErr) {
...
func (db *OceanBaseDialect) IsDeadlock(err error) bool {
	return db.isThisError(err, mysqlerr.ER_LOCK_DEADLOCK)
}

// UpsertSQL returns the upsert sql statement for MySQL dialect
func (db *OceanBaseDialect) UpsertSQL(tableName string, keyCols, updateCols []string) string {
	q, _ := db.UpsertMultipleSQL(tableName, keyCols, updateCols, 1)
	return q
}

func (db *OceanBaseDialect) UpsertMultipleSQL(tableName string, keyCols, updateCols []string, count int) (string, error) {
	if count < 1 {
...
func (db *OceanBaseDialect) Lock(cfg LockCfg) error {
...
func (db *OceanBaseDialect) Unlock(cfg LockCfg) error {
...
func (db *OceanBaseDialect) GetDBName(dsn string) (string, error) {
...
func (db *OceanBaseDialect) CreateTableSQL(table *Table) string {
