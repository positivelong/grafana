...
func (db *DmDialect) ColString(col *Column) string {
	...
	if db.dialect.ShowCreateNull() && !col.IsPrimaryKey {
		if col.Nullable {
			sql += "NULL "
		} else {
			sql += "NOT NULL "
...
func (db *DmDialect) IndexCheckSQL(tableName, indexName string) (string, []interface{}) {
	args := []interface{}{tableName, indexName, db.Schema}
...
func (db *DmDialect) ColumnCheckSQL(tableName, columnName string) (string, []interface{}) {
	args := []interface{}{tableName, columnName, db.Schema}
	sql := "SELECT 1 FROM ALL_INDEXES WHERE TABLE_NAME = ? AND INDEX_NAME = ? AND OWNER = ?"
	return sql, args
}

func (db *DmDialect) CleanDB(engine *xorm.Engine) error {
	...
	for _, table := range tables {
		...
		default:
			if _, err := sess.Exec(fmt.Sprintf(`drop table %s;`, table.Name)); err != nil {
				return errors.Wrapf(err, "failed to delete table %q", db.Quote(table.Name))
			}
		}
	}
...
func (db *DmDialect) TruncateDBTables(engine *xorm.Engine) error {
	tables, err := engine.DBMetas()
	if err != nil {
		return err
	}
	...
	for _, table := range tables {
		...
		case "dashboard_acl":
			...
			if _, err := sess.Exec(fmt.Sprintf("DELETE FROM %v WHERE dashboard_id != -1 AND org_id != -1;", db.Quote(table.Name))); err != nil {
				return errors.Wrapf(err, "failed to truncate table %q", table.Name)
			}
			if _, err := sess.Exec(fmt.Sprintf("ALTER TABLE %v AUTO_INCREMENT = 3;", db.Quote(table.Name))); err != nil {
				return errors.Wrapf(err, "failed to reset table %q", table.Name)
			}
		default:
...
func (db *DmDialect) DropIndexSQL(tableName string, index *Index) string {
	quote := db.dialect.Quote
	name := index.XName(tableName)
	return fmt.Sprintf("DROP INDEX %s.%s", quote(db.Schema), quote(name))
}

func (db *DmDialect) CreateIndexSQL(tableName string, index *Index) string {
	quote := db.dialect.Quote
	var unique string
	...
	return fmt.Sprintf("CREATE%s INDEX %v ON %s.%v (%v);", unique, quote(idxName), quote(db.Schema), quote(tableName), strings.Join(quotedCols, ","))
}

func (db *DmDialect) UpsertMultipleSQL(tableName string, keyCols, updateCols []string, count int) (string, error) {
	if count < 1 {
...
func (db *DmDialect) BatchSize() int {
	return 1000
}
