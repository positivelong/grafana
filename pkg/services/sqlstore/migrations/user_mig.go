...
func addUserMigrations(mg *Migrator) {
	...
	mg.AddMigration("Update is_service_account column to nullable",
		NewRawSQLMigration("").
			...
			Mysql("ALTER TABLE user MODIFY is_service_account BOOLEAN DEFAULT 0;").OceanBase("ALTER TABLE user MODIFY is_service_account BOOLEAN DEFAULT 0;"))
	...
	mg.AddMigration("Update uid column values for users", NewRawSQLMigration("").
		...
		Mysql("UPDATE user SET uid=concat('u',lpad(id,9,'0')) WHERE uid IS NULL;").OceanBase("UPDATE user SET uid=concat('u',lpad(id,9,'0')) WHERE uid IS NULL;"))
